  ...
  return {
    ...
    profileUpdateRequests: (profileUpdateRequestsResult.data ?? []).map(
      (row): MemberProfileUpdateRequest => ({
        ...
        area: row.area,
        photoUrl: row.photo_url ?? undefined,
        requestedFamilyMembers: (row.requested_family_members ?? []).map((familyMember: Record<string, unknown>) => ({
          id: String(familyMember.id),
          memberId: String(familyMember.member_id ?? row.member_id),
          relationshipType: String(familyMember.relationship_type) as MemberProfileUpdateRequest['requestedFamilyMembers'][number]['relationshipType'],
          fullName: String(familyMember.full_name ?? ''),
          nic: String(familyMember.nic ?? ''),
          dateOfBirth: String(familyMember.date_of_birth ?? ''),
          address: String(familyMember.address ?? ''),
          photoUrl: familyMember.photo_url ? String(familyMember.photo_url) : undefined,
          createdAt: String(familyMember.created_at ?? row.created_at),
          updatedAt: String(familyMember.updated_at ?? row.created_at),
        })),
        status: row.status,
...
export const syncAppDataToSupabase = async (data: AppDataState) => {
  ...
  const profileUpdateRequests = data.profileUpdateRequests.map((item) => ({
    ...
    requested_family_members: item.requestedFamilyMembers.map((familyMember) => ({
      id: familyMember.id,
